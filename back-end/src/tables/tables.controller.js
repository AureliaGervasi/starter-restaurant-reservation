...
async function validateReservationId(request, response, next) {
    ...
    if (!reservation) {
        return next({ status: 400, message: `reservation_id ${reservation_id} does not exist` });
    }
...
async function update(request, response) {
    await service.occupy(response.locals.table.table_id, response.locals.reservation.reservation_id);
    await service.updateReservation(response.locals.reservation.reservation_id, "seated");

    response.status(200).json({ data: { status: "seated" } });
}

async function destroy(request, response) {
    await service.updateReservation(response.locals.table.reservation_id, "finished");
